...
def get_subGraph(type,name):
    ...
    node = pd.read_csv("../covid_data/processed_data/node_0310.csv",
                       dtype={"0": np.float64, "1": np.float64, "2": np.float64, "3": np.float64, "4": np.float64})
    node_label_file = pd.read_csv("../covid_data/processed_data/node_0325.csv",
                       dtype={"0": np.float64, "1": np.float64, "2": np.float64, "3": np.float64, "4": np.float64})
    ...
    node_label = node_label_file.iloc[node_name_list, 1].to_numpy()
    ...
    node_label = torch.from_numpy(node_label).double().unsqueeze(dim=1)
